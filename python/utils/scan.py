...
class scanEFT:
    ...
    def getScan(self):
        ...
        if isinstance(self.poi, str):
            ...
            n = t.Draw(to_draw.Data(), "deltaNLL<{} && deltaNLL>{}".format(
                self.upper, self.lower), "l")

            if n <= 1:
                print("[ATTENTION] no likelihood for {}".format(self.poi))
                return

            # removing first element (0,0)
            x = np.ndarray((n), 'd', t.GetV2())[1:]
            # removing first element (0,0)
            y_ = np.ndarray((n), 'd', t.GetV1())[1:]
            ...
            if not self.isNuis:
                ...
                ROOT.TColor.CreateGradientColorTable(
                    4, __stops, __red, __green, __blue, 50, 0.8)
            ...
            cont_graphs = [deepcopy(conts.At(i)) for i in range(len(conts))]

            self.contours = cont_graphs
