...
class AnaliticAnomalousCouplingMorphing_comb(PhysicsModel):
    ...
    def doParametersOfInterest(self):
        ...
        for _bin_name in self.bin_ops_map:
            ...
            if self.numOperators == 0:
              ...
              # print(
              #   f'{sm_func_name}("@0*(1-(' +
              #                               "+".join(["@{}*@{}".format(i+1,i+1) for i in range(len(active_ops))])  +
              #                               "-@" + "-@".join([str(i+1)+"*@"+str(j+1)  for i in range(len(active_ops)) for j in range(len(active_ops)) if i<j ]) +
              #                               "))\",r," + "k_" + ", k_".join([str(active_ops[i]) for i in range(len(active_ops))]) + ")"
              # )
              self.private_factory(
                f'{sm_func_name}("@0*(1-(' 
                + "+".join(["@{}*@{}".format(i+1,i+1) for i in range(len(active_ops))])
            ...
            if self.numOperators == 1:
              ...
              sm_l_q_func_name = f"expr::func_sm_linear_quadratic_{bin_name}_" + str(active_ops[0])
              
              # print (f"{sm_l_q_func_name}" 
              #   + '("@0*(' 
              #   + '0.5*@1*(1+@1)'
              #   + ')",r,k_' 
              #   + str(active_ops[0])
              ...
              for operator in range(0, self.numOperators):
                ...
                sm_l_q_func_name = f"expr::func_sm_linear_quadratic_{bin_name}_" + str(active_ops[operator])
                
                # print(
                #   f'{sm_l_q_func_name}' 
                #   + '("@0*("' 
                #   + '0.5*@1*(1+@1-' 
                #   + '-'.join(['2*@{}'.format(j+1) for j in range(1, len(active_ops))]) 
                #   + ')' 
                #   + ')",r,k_' 
            ...
            if self.numOperators == 1:
              ...
              sm_ml_q_func_name = f"expr::func_sm_minuslinear_quadratic_{bin_name}_" + str(active_ops[0])
              ...
              for operator in range(0, self.numOperators):
                ...
                sm_ml_q_func_name = f"expr::func_sm_minuslinear_quadratic_{bin_name}_" + str(active_ops[operator])
            ...
            if self.numOperators > 1:
              for operator in range(0, self.numOperators):
                for operator_sub in range(operator+1, self.numOperators):
                  ...
                  sm_mixed_func_name = f'expr::func_sm_linear_quadratic_mixed_{bin_name}_' + str(active_ops[operator]) + '_' + str(active_ops[operator_sub])

                  # print(
                  #   f'{sm_mixed_func_name}'
                  #   +  '("@0*@1*@2",r,k_'
                  #   + str(self.active_ops[operator]) 
                  #   + ',k_"' 
                  #   + str(self.active_ops[operator_sub])
                  #   + ')'
                  # )
...
#
    ...
    def getYieldScale_(self,bin,process):
        ...
        for operator in range(0, self.numOperators):
            if (
                process == "w1_" + str(active_ops[operator]) 
                or process.endswith("_w1_" + str(active_ops[operator])) 
            ):
            ...
            if (
                process == "wm1_" + str(active_ops[operator]) 
                or process.endswith("_wm1_" + str(active_ops[operator])) 
            ):
                return f'func_sm_minuslinear_quadratic_{bin_name}_' + str(
                    active_ops[operator]
                )
            ...
            for operator_sub in range(operator+1, self.numOperators):
                ...
                if (
                    process == 'w11_' + str(active_ops[operator])  + '_' + str(active_ops[operator_sub])
                    or 
                    process.endswith('_w11_' + str(active_ops[operator]) + '_' + str(active_ops[operator_sub])
                    )
                ):
                  
                  return f'func_sm_linear_quadratic_mixed_{bin_name}_' + str(active_ops[operator]) + '_' + str(active_ops[operator_sub])
                            
                if (
                    process == 'w11_' + str(active_ops[operator_sub])  + '_'  + str(active_ops[operator])
                    or 
                    process.endswith('_w11_' + str(active_ops[operator_sub]) + '_' + str(active_ops[operator])
                    )
                ):
...
analiticAnomalousCouplingMorphing_comb = AnaliticAnomalousCouplingMorphing_comb()
